...
See `comparable-games.csv` and `frame-analysis.csv`. QX-R4-R00 uses official-page frames only as a structural sample. Quantitative 12-frame analysis belongs to QX-R4-R01.
...
Fallback: remove the QX-R4-R00 commit and return to the accepted R3 candidate.
...
- For future quality tasks: can a first-time human describe the Candidate improvement under the same conditions? QX-R4-R00 itself changes process only and does not claim a gameplay-quality Human pass.

## Gate status

- Research: passed by `npm run research:validate -- QX-R4-R00`
- Implementation: isolated in `codex/qx-r4-r00-evidence-protocol`
- Completion: governed by QX-R4-R00 explicit tests and independent review; generic quality-task completion remains fail-closed without human evidence.
