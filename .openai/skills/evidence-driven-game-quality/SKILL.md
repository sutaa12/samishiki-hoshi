...
- Acceptance scope is exactly QX-R5-001 through QX-R5-007, plus the one-time QX-R4-R01 research-only migration. The repository validator is an integrity checker, not a cryptographic identity authority or a visual-semantic model: external orchestration must actually run the three isolated reviewers and preserve invocation provenance. Treat forged review JSON or arbitrary imagery as a trust-root breach, not as proof that local pixel heuristics can authenticate an AI process.
...
Telemetry must record at least 15.000 seconds and contain one indexed, uniformly frame-timed `distance_mm` sample for every decoded video frame with strictly increasing time and distance across the whole clip. It must also include a Ring probe whose after-area is at least 4x its positive before-area within 2.5 seconds, and an in-viewport steering probe showing at least 12px movement in 0–100ms plus at least 10% viewport movement completed within 300ms. Maximum continuous-still duration is bounded to 0–500ms and encounter order is `ring → obstacle → node`. The passed Event ledger contains exactly one strictly chronological `ring_success → rock_avoid|rock_contact → node_pulse → progress_update` sequence inside the telemetry interval, with no failed or duplicate semantic event. Review receipts cannot contain a contradictory verdict/outcome beside `pass: true`. The remediation receipt must use the canonical structured Page 19 first-fix and prohibited-first routing for `motion`, `player`, `objective`, `pulse`, and `progress`, plus a non-filler observation, even when all three reviews pass and the decision is `accept`.
