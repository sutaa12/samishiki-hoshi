...
- AI Binary completion requires one digest-bound moving video, telemetry receipt, event ledger, remediation decision, and exactly three distinct artifact-only blind AI reviews. The validator must decode the video itself, confirm at least 15 seconds of video frames, and confirm multiple distinct sampled frames; a self-declared media receipt or arbitrary bytes are insufficient. Every review must bind the same Task, Source SHA-256, Build SHA-256, and Video SHA-256; every binary answer and `pass` must be true; Reviewer IDs and punctuation-insensitive free-description fingerprints must be distinct; descriptions may not copy the expected answer.
...
Telemetry must record at least 15 seconds and contain one indexed, timestamped `distance_mm` sample for every recorded frame with strictly increasing time and distance. It must also include a Ring probe whose after-area is at least 4x its positive before-area, and a steering probe showing input response in 0–100ms plus at least 10% viewport movement completed within 300ms. Maximum continuous-still duration is bounded to 0–500ms and encounter order is `ring → obstacle → node`. Event ledger entries must remain within the telemetry interval, carry monotonic `at_ms` values, and cover `ring`, `obstacle`, `node`, and `progress`. The remediation receipt must preserve distinct, substantive Page 19 routing for `motion`, `player`, `objective`, `pulse`, and `progress`, even when all three reviews pass and the decision is `accept`.
