...
- AI Binary completion requires one digest-bound moving video, telemetry receipt, event ledger, remediation decision, and exactly three distinct artifact-only blind AI reviews. The validator must decode the video itself, confirm at least 15.000 seconds, compare telemetry count with decoded frames, and reject static plates through sampled-frame continuity; a self-declared media receipt, arbitrary bytes, or slideshow are insufficient. Every review must bind the same Task, Source SHA-256, Build SHA-256, and Video SHA-256; every binary answer and `pass` must be true; Reviewer IDs and punctuation-insensitive free-description fingerprints must be distinct; a description may not contain the complete expected-answer fingerprint even with added prefixes or suffixes.
