...
- AI Binary completion uses `npm run research:validate -- <task-id> --stage complete --acceptance ai-binary` and requires telemetry, a directly decoded continuously moving video of at least 15.000 seconds, a semantic event ledger, the exact Page 19 remediation routing map, and three distinct artifact-only blind AI reviews bound to one source, build, and video digest. Telemetry frame count must equal decoded video frames and contain the complete monotonic distance trace; Ring area must grow at least 4x within 2.5 seconds; steering must move at least 12px within 100ms and at least 10% of the viewport within 300ms.
